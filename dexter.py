...
def eliminate_target(target_ip):
  if not is_valid_ip(target_ip):
    print(
        f"{RED}[-] ERROR: [{target_ip}] is not a valid IP address! Check your"
        f" input.{RESET}\n"
    )
    return
  ...
  print(
      f"{YELLOW}[*] Preparing the kill room. Laying down the plastic sheets for"
      f" IP: {target_ip}...{RESET}"
  )
  ...
  if result == 0:
    print(
        f"\n{RED}[☠️] EXECUTION SUCCESSFUL! Target [{target_ip}] has been sliced"
        f" from the network and dropped into the deep ocean. 🌊{RESET}"
    )

    repo_updated = save_to_banned_repository(target_ip)
    if repo_updated:
      print(
          f"{GREEN}[+] IP successfully added to local repository:"
          f" {BANNED_REPO_FILE}{RESET}"
      )
    else:
      print(
          f"{YELLOW}[!] Note: IP was dropped, but it was already recorded in"
          f" the repository.{RESET}"
      )
    ...
    try:
      with open(kill_room_log, "a") as file:
        time_now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        file.write(
            f"[{time_now}] [ELIMINATED] -> Source IP: {target_ip} dropped"
            " silently.\n"
        )
      print(
          f"{GREEN}[+] Threat indexed properly in system log:"
          f" {kill_room_log}{RESET}\n"
      )
    except PermissionError:
      print(
          f"{YELLOW}[!] WARNING: Rule applied, but system log access"
          f" denied.{RESET}\n"
      )
  else:
    print(
        f"{RED}[-] ERROR: The target {target_ip} slipped away! Ensure you are"
        f" running with sudo/root privileges.{RESET}\n"
    )
...
if __name__ == "__main__":
  ...
  if len(sys.argv) < 2:
    ...
    if os.path.exists(BANNED_REPO_FILE):
      print(
          f"{CYAN}[*] Current Banned IPs Repository ({BANNED_REPO_FILE}):{RESET}"
      )
      with open(BANNED_REPO_FILE, "r") as f:
